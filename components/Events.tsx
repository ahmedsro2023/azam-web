...
const events: EventItem[] = [
  { 
    ...
    date: 'OCT 24, 2024', 
    image: 'https://images.unsplash.com/photo-1512453979798-5ea904ac6666?q=80&w=1200&auto=format&fit=crop' 
  },
  { 
    id: 2, 
    title: 'Midnight Illusions', 
    location: 'London, UK', 
    date: 'NOV 05, 2024', 
  ...
  { 
    id: 4, 
    title: 'Private Mansion Show', 
    location: 'Monaco', 
    date: 'JAN 01, 2025', 
    image: 'https://images.unsplash.com/photo-1559528726-253c528ae449?q=80&w=1200&auto=format&fit=crop' 
  },
];
        ...
        {events.map((event, index) => (
          ...
          >
             ...
             <div className="absolute inset-0 z-0 opacity-0 group-hover:opacity-40 transition-opacity duration-700">
                <img src={event.image} alt="bg" className="w-full h-full object-cover grayscale mix-blend-screen" />
                <div className="absolute inset-0 bg-gradient-to-r from-black via-black/50 to-transparent"></div>
             </div>

             <div className="relative z-10 flex flex-col md:flex-row items-baseline gap-6 md:gap-12 w-full">
                <span className="text-neutral-600 text-sm font-mono transition-colors group-hover:text-yellow-500">0{index + 1}</span>
             ...
             <div className="relative z-10 flex flex-col md:items-end gap-2 mt-4 md:mt-0 opacity-60 group-hover:opacity-100 transition-opacity">
                <div className="flex items-center gap-2 text-sm uppercase tracking-wider">
                   <MapPin size={14} className="group-hover:text-yellow-500 transition-colors" />
             ...
             <div className="absolute right-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-x-10 group-hover:translate-x-0 z-20">
                <span className="text-xs uppercase tracking-widest border border-white px-4 py-2 hover:bg-white hover:text-black transition-colors">Get Tickets</span>
             </div>
          </motion.div>
        ))}
      </div>
